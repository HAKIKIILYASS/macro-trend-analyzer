  ...
  return <SidebarProvider>
      <div className="min-h-screen flex w-full bg-gradient-to-br from-gray-50 via-blue-50/30 to-green-50/30 dark:from-gray-900 dark:via-blue-950/20 dark:to-green-950/20 transition-all duration-500">
        ...
        <div className="fixed top-4 left-4 right-4 z-50 flex justify-between items-center pointer-events-none">
          <Button variant="outline" size="sm" onClick={() => setSidebarOpen(!sidebarOpen)} className="pointer-events-auto bg-white/90 hover:bg-white backdrop-blur-sm shadow-lg border-gray-200 text-zinc-950">
        ...
        <div className="fixed left-0 top-0 h-screen z-40">
          <RecentActivitySidebar open={sidebarOpen} setOpen={setSidebarOpen} onLoadScore={handleLoadScore} onCompare={handleCompare} />
        </div>
        ...
        <div className={`flex-1 p-4 pt-20 transition-all duration-200 ${sidebarOpen ? "md:ml-72" : ""}`}>
          <div className="max-w-7xl mx-auto">
            ...
            {results && <div className="mb-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
              <div className="h-full animate-fade-in">
                <CentralBankInput value={data.cb_hawkish_index} onChange={value => updateData('cb_hawkish_index', value)} />
              </div>
              <div className="h-full animate-fade-in">
                <InflationInput cpi={data.cpi} cpi_target={data.cpi_target} cpi_3m_change={data.cpi_3m_change} onCpiChange={value => updateData('cpi', value)} onTargetChange={value => updateData('cpi_target', value)} onChange3M={value => updateData('cpi_3m_change', value)} />
              </div>
              <div className="h-full animate-fade-in">
                <LaborMarketInput current_nfp={data.current_nfp} nfp_12m_values={data.nfp_12m_values} onCurrentChange={value => updateData('current_nfp', value)} onValuesChange={values => updateData('nfp_12m_values', values)} />
              </div>
              <div className="h-full animate-fade-in">
                <RiskSentimentInput credit_spread_1m_change={data.credit_spread_1m_change} vix={data.vix} onCreditChange={value => updateData('credit_spread_1m_change', value)} onVixChange={value => updateData('vix', value)} />
              </div>
              <div className="h-full animate-fade-in">
                <PMIInput pmi={data.pmi} pmi_3y_values={data.pmi_3y_values} onPmiChange={value => updateData('pmi', value)} onValuesChange={values => updateData('pmi_3y_values', values)} />
              </div>
              <div className="h-full animate-fade-in">
                <CurrentAccountInput ca_gdp={data.ca_gdp} ca_5y_values={data.ca_5y_values} onCurrentChange={value => updateData('ca_gdp', value)} onValuesChange={values => updateData('ca_5y_values', values)} />
              </div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              <div className="lg:col-span-2 animate-scale-in">
                <GeopoliticalInput gpr={data.gpr} gpr_3y_values={data.gpr_3y_values} onGprChange={value => updateData('gpr', value)} onValuesChange={values => updateData('gpr_3y_values', values)} />
              </div>
              <Card className="shadow-lg border border-gray-200 dark:border-gray-700 animate-scale-in bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
                <CardContent className="h-full flex items-center justify-center p-6">
                  <div className="w-full space-y-4">
                    <Button onClick={calculateScores} className="w-full h-12 text-lg font-semibold bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-lg">
                      Calculate Macro Score
                    </Button>
                    
                    {results && <Button onClick={() => setShowSaveDialog(true)} variant="outline" className="w-full h-12 text-base font-semibold border-2 hover:bg-primary/5 transition-all duration-200">
            ...
            <SaveScoreDialog isOpen={showSaveDialog} onSave={handleSaveScore} onCancel={() => setShowSaveDialog(false)} />
          </div>
        </div>
      </div>
    </SidebarProvider>;
